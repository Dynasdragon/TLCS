...
Usage - sources:
    py detect.py --weights ../assets/best_v1.pt --source ../assets/image.jpg
...
pytesseract.pytesseract.tesseract_cmd = 'C:\\Users\\xdark\\AppData\\Local\\Programs\\Tesseract-OCR\\tesseract.exe'
...
def run(
        ...
        data=ROOT / 'data/data_ocr.yaml',  # dataset.yaml path
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_obj:
                        ...
                        if int(cls) == 15:
                            ...
                            text = pytesseract.image_to_string(filepath, config='txt --psm 8')
                            #text = pytesseract.image_to_string('C:\\Users\\xdark\\source\\repos\\TLCS\\src\\backend\\runs\\detect\\exp11\\lp0test.jpg', config='--psm 2')
                            clean_string = [s for s in text if s.isalnum()]

                            # rejoin intermediate list into a string
                            clean_string = "".join(clean_string)
                            print(clean_string)
                            f = open("C:\\Users\\xdark\\source\\repos\\TLCS\\src\\assets\\lp.txt", "w")
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/data_ocr.yaml', help='(optional) dataset.yaml path')
